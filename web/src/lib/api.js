...
async function jfetch(path, options = {}) {
  ...
  const res = await fetch(`${API_BASE}${path}`, {
    ...options,
    headers,
    credentials: "include", // ✅ IMPORTANT: send/receive cookies for OTP + owner sessions
  });
...
export const api = {
  ...
  createRequest: (payload) => jfetch("/requests", { method: "POST", body: JSON.stringify(payload) }),
  ...
  sendMessage: (conversationId, sender, content, ownerToken = null) =>
    jfetch(`/conversations/${conversationId}/messages`, {
      method: "POST",
      // Legacy header kept for backward compatibility; with cookie-session owner auth you can remove this later.
  ...
  setClientAvatar: (clientId, url) =>
    jfetch(`/clients/${clientId}/avatar`, { method: "PATCH", body: JSON.stringify({ url }) }),

  setPetAvatar: (petId, url) =>
    jfetch(`/pets/${petId}/avatar`, { method: "PATCH", body: JSON.stringify({ url }) }),

  addPetPhoto: (petId, url, caption) =>
    jfetch(`/pets/${petId}/photos`, { method: "POST", body: JSON.stringify({ url, caption }) }),


  updateClientProfile: (name, phone) =>
    jfetch("/clients/me", { method: "PATCH", body: JSON.stringify({ name, phone }) }),


  createPetMe: (payload) =>
  jfetch("/pets/me", { method: "POST", body: JSON.stringify(payload) }),
  ...
  // -------------------------
  // Client OTP Auth (cookie)
  // -------------------------
  requestClientOtp: (email) =>
    jfetch("/auth/client/request-otp", { method: "POST", body: JSON.stringify({ email }) }),

  verifyClientOtp: (email, code) =>
    jfetch("/auth/client/verify-otp", { method: "POST", body: JSON.stringify({ email, code }) }),

  clientMe: () => jfetch("/auth/client/me"),

  clientLogout: () => jfetch("/auth/client/logout", { method: "POST" }),

  // -------------------------
  // Owner Auth (cookie)
  // -------------------------
  ownerLogin: (password) =>
    jfetch("/auth/owner/login", { method: "POST", body: JSON.stringify({ password }) }),

  ownerLogout: () => jfetch("/auth/owner/logout", { method: "POST" }),

  holidays: () => jfetch("/holidays"),

  createMeetGreetCheckout: (requestId) =>
    jfetch(`/payments/meet-greet/${requestId}/checkout`, { method: "POST" }),

  createBookingCheckout: (requestId) =>
    jfetch(`/payments/request/${requestId}/checkout`, { method: "POST" }),

  devPayMeetGreet: (requestId) =>
    jfetch(`/payments/meet-greet/${requestId}/dev-pay`, { method: "POST" }),

  devPayRequest: (requestId) =>
    jfetch(`/payments/request/${requestId}/dev-pay`, { method: "POST" }),
